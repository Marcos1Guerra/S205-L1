...
// Inserir os cards dinamicamente
function renderEvents(eventos) {
  eventos.forEach((event, index) => {
      const card = document.createElement('div');
      card.classList.add('card');
      if (index !== 0) card.style.display = 'none'; // Apenas o primeiro visível
      card.innerHTML = `
...
// Chama a função para renderizar os eventos na tela
renderEvents(eventos);

// Implementação do carrossel para exibição de um card por vez
let currentIndex = 0;
